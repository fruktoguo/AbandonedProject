...
namespace YuoTools
{
    public static class YuoLog
    {
        ...
        public static T Log<T>(this T obj)
        {
            ...
            Debug.Log($"{ExtensionTag.LogSetColor(YuoStrColor.矢车菊的蓝色).LogSetBold()}{obj.ToString().LogSetBold().LogSetColor(YuoStrColor.深紫罗兰的蓝色)}");
            return obj;
        }
        ...
        {
            if (!OpenDebug) return obj;
            Debug.Log($"{ExtensionTag.LogSetColor(YuoStrColor.矢车菊的蓝色).LogSetBold()}{obj.ToString().LogSetBold().LogSetColor(Color)}");
            return obj;
        }
        ...
        public static List<T> LogAll<T>(this List<T> objs)
        {
            ...
            $"该 {typeof(T).ToString().LogSetColor(YuoStrColor.钢蓝) } 集合 的长度为 [{objs.Count.ToString().LogSetColor(YuoStrColor.深粉色)}]".Log();
            for (int i = 0; i < objs.Count; i++)
            {
                $"第 [{(i + 1).ToString().LogSetColor(YuoStrColor.深粉色)}] 个元素为 {objs[i].ToString().LogSetColor(YuoStrColor.耐火砖)}".Log();
            }
            return objs;
        }
        ...
        {
            if (!OpenDebug) return objs;
            $"该 {typeof(T).ToString().LogSetColor(YuoStrColor.钢蓝)} 数组 的长度为 [{objs.Length.ToString().LogSetColor(YuoStrColor.深粉色)}]".Log();
        ...
        {
            if (!OpenDebug) return objs;
            $"该 【Key :{typeof(T1).ToString().LogSetColor(YuoStrColor.钢蓝)} , Value : {typeof(T2).ToString().LogSetColor(YuoStrColor.钢蓝)} 】 字典 共有[{objs.Count.ToString().LogSetColor(YuoStrColor.深粉色)}] 个元素 ".Log();
            int i = 0;
            foreach (var item in objs)
            {
                $"第 [{(i + 1).ToString().LogSetColor(YuoStrColor.深粉色)}] 个元素为 {item.ToString().LogSetColor(YuoStrColor.耐火砖)}".Log();
                i++;
            }
            return objs;
        }
