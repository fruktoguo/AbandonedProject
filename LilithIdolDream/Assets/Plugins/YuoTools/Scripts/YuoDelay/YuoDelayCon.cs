...
namespace YuoTools
{
    public class YuoDelayCon : SingletonMono<YuoDelayCon>
    {
        ...
        private IEnumerator IYuoDelay<T>(T obj, YuoDelayMod mod)
        {
            ...
            {
                yield return YuoWait.GetWait(mod.DelayTime);
        ...
        private IEnumerator IYuoDelayRealtime<T>(T obj, YuoDelayMod mod)
        {
            ...
            {
                yield return YuoWait.GetWaitRealtime(mod.DelayTime);
                while (mod.ExtraDelayTime.Count > 0)
                {
                    Temp.Float = mod.ExtraDelayTime[0];
                    mod.ExtraDelayTime.RemoveAt(0);
                    yield return YuoWait.GetWaitRealtime(Temp.Float);
                }
            }
            if (!mod.End)
            {
    ...
    public class YuoDelayMod
    {
        ...
        {
            if (name.Equals(Name))
                YuoDelayCon.Instance.StopCor(this);
        }
    ...
    public static class Delay
    {
        ...
        {
            YuoDelayCon.Instance.StopForce(mod);
        }
