    ...
    public static class SpawnUICode
    {
            ...
            Transform transform, bool findThis = false)
        {
            ...
            if (false)
            {
                ...
                {
                    if (item.name.StartsWith(SpawnUICodeConfig.GeneralUITag))
                    {
                        generals.Add(item);
                        generalTemps.AddRange(FindAll(item));
                    }

                    if (item.name.StartsWith(SpawnUICodeConfig.ChildUITag))
                    {
                        children.Add(item);
                        childTemps.AddRange(FindAll(item));
                    }

                    if (item.name.StartsWith(SpawnUICodeConfig.VariantChildUITag))
                    {
                        variantChildren.Add(item);
                        variantChildTemps.AddRange(FindAll(item));
                    }
                }
                ...
                //将变体子组件移除
                foreach (var item in variantChildTemps)
                {
                    trans.Remove(item);
                }
            }
            ...
            foreach (var child in children)
            {
                ...
                string get = "{\n\t\t\tget" +
                             ...
                             $"\n\t\t\t\t\tWorld.RunSystem<IUICreate>(mChild_{name});" +
                             "\n\t\t\t\t}" +
                             $"\n\t\t\t\treturn " + $"mChild_{name}" + ";\n\t\t\t}\n\t\t}";
            ...
            foreach (var variantChild in variantChildren)
            {
                ...
                string get = "{\n\t\t\tget" +
                             ...
                             $"\n\t\t\t\t\tWorld.RunSystem<IUICreate>(mChild_{name}.Entity);" +
                             "\n\t\t\t\t}" +
                             $"\n\t\t\t\treturn " + $"mChild_{name}" + ";\n\t\t\t}\n\t\t}";
